...
export default function LeadershipSection({ leaders }: LeadershipSectionProps) {
  // If no leaders, don't render the section
  if (!leaders || leaders.length === 0) {
    return null;
  }

  // Split leaders into rows of 5
  const rows: LeadershipMember[][] = [];
  for (let i = 0; i < leaders.length; i += 5) {
    rows.push(leaders.slice(i, i + 5));
  }
      ...
      <div className="w-full max-w-[1100px] mx-auto flex flex-col items-center">
        ...
        <div className="w-full space-y-9">
          {rows.map((row, rowIndex) => (
            <div
              key={rowIndex}
              className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-x-7 gap-y-9 ${
                row.length < 5 ? "justify-items-center" : ""
              }`}
            >
