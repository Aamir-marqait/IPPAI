...
export default function ArticlesClient({ articles }: ArticlesClientProps) {
  ...
  const filteredArticles = articles.filter((article) => {
    ...
    const matchesCategory =
      selectedCategories.length === 0 ||
      selectedCategories.some((category) => articleCategories.includes(category));
          ...
          <div className="flex flex-col sm:flex-row gap-4 lg:min-w-[400px]">
            ...
            {allCategories.length > 0 && (
              <div className="relative dropdown-container">
                ...
                {isDropdownOpen && (
                  <div
                    className="absolute right-0 mt-2 w-64 bg-white rounded-lg border border-gray-200 z-10 max-h-80 overflow-y-auto"
                    style={{ boxShadow: "0px 0px 7.2px 0px #00000026" }}
                  >
                    <div className="p-3">
                      <h3 className="font-work-sans font-normal text-base leading-[100%] text-gray-900 mb-2">
                        Categories
                      </h3>
                      {allCategories.map((category) => (
                        <label key={category} className="flex items-center py-2 cursor-pointer">
                          <input
                            ...
                            className="h-4 w-4 text-[#D3363B] focus:ring-[#D3363B] border-gray-300 rounded"
                          />
                          <span className="ml-3 font-work-sans font-normal text-base leading-[100%] text-gray-700">
                            {category}
                          </span>
                        </label>
                      ))}
